...
**Phase 2 (Flux migration) — design-locked 2026-04-26, code not yet shipped.** A Flux Dev fp16 + Flat Cartoon Style LoRA + ControlNet Union Pro + XLabs Flux IP-Adapter v2 + img2img stack will replace the current SD 1.5 + AnyLoRA + DWPose / lineart-fallback stack across phases 2a–2g. Future Node 7 CLI will gain `--workflow {v1,v2}` (default `v2` after Phase 2c lands) and `--precision {fp16,fp8}` (default `fp16` for A100 80GB; `fp8` fallback for 4090-class GPUs). The Phase 1 stack stays callable via `--workflow=v1` for the entire 6-month deprecation window. See `CLAUDE.md` "Node 7 v2 — locked decisions" for the full 14-decision walkthrough and `docs/PLAN.md` "NODE 7 v2 — Phase 2 Flux migration" for the implementation roadmap.
