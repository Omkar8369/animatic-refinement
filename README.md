...
| 7 | AI-Powered Pose Refinement | **Phase 1 DONE (live-verified 2026-04-25). Phase 2a SHIPPED 2026-04-26. Phase 2b SHIPPED 2026-04-27. Phase 2c-2g pending.** Phase 1: 47 tests pass; live-verified on RunPod 4090 (DWPose + lineart-fallback routes both produce clean output on the synthetic smoke fixture). Bringup notes for the runpod-slim pod image captured in `tools/POD_NOTES_runpod_slim.md`. **Phase 2a (2026-04-26):** Flux + Style LoRA + ControlNet Union Pro integration in `workflow_flux_v2.json` (16 locked Phase 2 node IDs handling both DWPose and lineart-fallback routes via node 51 class swap + node 61 SetUnionControlNetType); `models.json` schema bumped (additive deprecation fields; Phase 1 weights flipped to `deprecated:true` with 2026-10-26 removal); Phase 2 weight pins (Flux Dev fp16/fp8 + T5-XXL fp16/fp8 + CLIP-L + Flux VAE + Flat Cartoon Style v1.2 + ControlNet Union Pro); `runpod_setup.sh` honours `DOWNLOAD_DEPRECATED`; `--workflow {v1,v2}` + `--precision {fp16,fp8}` CLI flags on Node 7 + Node 11; `RefinedGeneration` + `CharacterSpec` gained Phase 2 fields (additive). **Phase 2b (2026-04-27):** 436 tests pass repo-wide (91 in test_node7.py — 47 Phase 1 + 44 Phase 2 — plus 4 in test_node2.py for the per-character LoRA fields; zero regressions). Wired XLabs Flux IP-Adapter v2 into `workflow_flux_v2.json` via 3 additive locked node IDs: 22 (`Load Flux IPAdatpter` — upstream class_type typo preserved), 23 (LoadImage for the reference COLOR crop from Node 6E), 24 (`Apply Flux IPAdapter`). KSampler's model input rewires from node 20 → node 24 (only existing-node change). `ip_scale=0.8` per locked decision #6. `models.json` gained `flux-ip-adapter-v2.safetensors` (~1 GB) + `clip-vit-large-patch14.safetensors` (~600 MB CLIP-L vision encoder) + `x-flux-comfyui` custom node clone. Reference image is the COLOR crop (NOT the DoG line-art) per locked decision #4. Phase 2c-2g pending: 2c (img2img@denoise=0.55, flip --workflow default to v2) → 2d (TMKOC style LoRA) → 2e (per-character LoRAs) → 2f (Node 5 background-line fix) → 2g (Node 6 simplification). |
...
**Phase 2a (Flux migration) — SHIPPED 2026-04-26. Phase 2b (XLabs Flux IP-Adapter) — SHIPPED 2026-04-27.** Node 7's CLI accepts `--workflow {v1,v2}` (default `v1` for safety; will flip to `v2` in Phase 2c) and `--precision {fp16,fp8}` (default `fp16` for A100 80GB; `fp8` fallback for 4090-class GPUs). v2 routes both human and non-human characters through a single `workflow_flux_v2.json` (Flux Dev + Flat Cartoon Style v1.2 LoRA + ControlNet Union Pro + **XLabs Flux IP-Adapter v2 for character identity**), with the SetUnionControlNetType node handling per-character `openpose` vs `lineart` switching. Phase 2b's IP-Adapter receives Node 6's COLOR reference crop and wraps the LoRA-conditioned UNET to inject character identity at `ip_scale=0.8` (locked decision #6). Phase 1 stays the default + fully callable until Phase 2c flips the default; both flags pass through Node 11 unchanged. Phase 1 weights are marked `deprecated:true` in `models.json` with a scheduled-removal date of 2026-10-26 (6-month rollback window); fresh pods skip them by default (saves ~6.5 GB) but `DOWNLOAD_DEPRECATED=true bash runpod_setup.sh` opts back in. **Try Phase 2 v2 in dry-run on the laptop:**

```bash
# Phase 2 v2 dry-run (exercises new manifest fields + workflow loader,
# never touches ComfyUI):
python run_node7.py \
    --node6-result <work>/node6_result.json \
    --queue        <input>/queue.json \
    --workflow=v2 --precision=fp16 --dry-run
...
Phases 2b–2g still pending: 2b (XLabs IP-Adapter) → 2c (img2img@denoise=0.55, flip default to v2) → 2d (custom-trained TMKOC style LoRA) → 2e (per-character LoRAs) → 2f (Node 5 background-line fix) → 2g (Node 6 simplification). See `CLAUDE.md` "Node 7 v2 — locked decisions" for the full 14-decision walkthrough and `docs/PLAN.md` "NODE 7 v2 — Phase 2 Flux migration" for the implementation roadmap.
