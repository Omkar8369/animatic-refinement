...
| 7 | AI-Powered Pose Refinement | **Phase 1 DONE (live-verified 2026-04-25). Phase 2a SHIPPED 2026-04-26 (Flux + Style LoRA + Union CN integration; default still v1 for safety). Phase 2b-2g pending.** Phase 1: 47 tests pass; live-verified on RunPod 4090 (DWPose + lineart-fallback routes both produce clean output on the synthetic smoke fixture). Bringup notes for the runpod-slim pod image (controlnet_aux symlink + `extra_model_paths.yaml` + `IPAdapter.weight_type` + DWPose-specific venv deps) captured in `tools/POD_NOTES_runpod_slim.md`. **Phase 2a (2026-04-26):** 429 tests pass repo-wide (84 in test_node7.py — 47 Phase 1 + 37 new Phase 2 — plus 4 new test_node2.py cases for the per-character LoRA fields; zero regressions). Shipped: `custom_nodes/node_07_pose_refiner/workflow_flux_v2.json` (single Flux + Style LoRA + Union CN workflow handling both DWPose and lineart-fallback routes via node 51 class swap + node 61 SetUnionControlNetType; 16 locked Phase 2 node IDs); `models.json` schema bumped (additive `deprecated`/`deprecatedSince`/`deprecatedReason`/`scheduledRemovalDate`/`precision` fields; Phase 1 weights flipped to `deprecated:true` with 2026-10-26 removal date; Phase 2 entries added: Flux Dev fp16/fp8 + T5-XXL fp16/fp8 + CLIP-L + Flux VAE + Flat Cartoon Style v1.2 + ControlNet Union Pro); `runpod_setup.sh` honours `DOWNLOAD_DEPRECATED` env var (default false skips deprecated weights and the deprecated SD 1.5 IPAdapter custom node, saves ~6.5 GB on fresh pods); CLI gained `--workflow {v1,v2}` (default `v1`; flip happens in Phase 2c) + `--precision {fp16,fp8}` (default `fp16` for A100 80GB) flags on both Node 7 and Node 11 (passthrough); `RefinedGeneration` + `CharacterSpec` schemas gained Phase 2 fields (additive, Phase 1 records/files load unchanged). Phase 2b-2g pending: 2b (XLabs IP-Adapter) → 2c (img2img@denoise=0.55, flip --workflow default to v2) → 2d (TMKOC style LoRA) → 2e (per-character LoRAs) → 2f (Node 5 background-line fix) → 2g (Node 6 simplification). |
