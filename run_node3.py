"""Local-friendly wrapper for Node 3's CLI.

On RunPod (and any standard Python install), `python -m pipeline.cli_node3 ...`
...
root never ends up on `sys.path` automatically, and `-m pipeline.cli_node3`
...
to `pipeline.cli_node3.main`, so it works identically in both environments:

    # Windows (embedded Python):
    "C:\\...\\python_embeded\\python.exe" run_node3.py --queue <q> --work-dir <w>

    # RunPod (standard Python):
    python run_node3.py --queue <q> --work-dir <w>
    # (or equivalently: python -m pipeline.cli_node3 --queue <q> --work-dir <w>)
"""
...
from pipeline.cli_node3 import main  # noqa: E402 - path fixup must happen first
