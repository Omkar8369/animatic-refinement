"""Local-friendly wrapper for Node 2's CLI.

On RunPod (and any standard Python install), `python -m pipeline.cli ...`
...
root never ends up on `sys.path` automatically, and `-m pipeline.cli`
...
to `pipeline.cli.main`, so it works identically in both environments:

    # Windows (embedded Python):
    "C:\\...\\python_embeded\\python.exe" run_node2.py --input-dir <path>

    # RunPod (standard Python):
    python run_node2.py --input-dir <path>
    # (or equivalently: python -m pipeline.cli --input-dir <path>)
"""
...
from pipeline.cli import main  # noqa: E402 - path fixup must happen first
