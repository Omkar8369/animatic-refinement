...
- **7v2-D. Phase 2d — Train TMKOC style LoRA.** Generate training data via Phase 2c output (or curated TMKOC frame extracts if legally clean). Train on A100 with ai-toolkit (ostris). Ship `tmkoc_style_v1.safetensors` to `models.json`. Replace generic `flat_cartoon_style_v12.safetensors` reference in `workflow_flux_v2.json`. Strength tuning per-shot via test runs.
...
- **7v2-F. Phase 2f — Fix Node 5 background-line detection bug.** Otsu fallback when bbox spans >70% of frame (catches the TMKOC failure mode where sketched backgrounds got picked up as one giant character bbox covering the entire frame). Manual region-of-interest hint via metadata. Doesn't touch Node 7 directly but is an upstream prerequisite for clean Phase 2c output on real client shots.
