...
- **6D. Angle matching** — scale-normalize the rough silhouette and each of the 8 reference-angle silhouettes (alpha masks) to the 128×128 canvas, centered on centroid, aspect-preserving padded. Score each `(rough, reference)` pair via the weighted multi-signal described above. **Phase 2g (2026-05-03):** the multi-signal scorer is RETAINED for `--angle-mode auto` but the production default is `--angle-mode front-only`, which overrides the silhouette-scoring winner with the `"front"` reference. IP-Adapter needs the character's FACE for identity learning; back-view crops produce generic character output. The override leaves a `phase2g_override = "front"` marker in `scoreBreakdown` so the operator can distinguish "Node 6 picked front via Phase 2g override" from "Node 6 picked front legitimately via auto-mode silhouette scoring".
...
- **7v2-Tune. Phase 2f-tuning — Adaptive iterative dilation before CC (DONE 2026-05-03).** Phase 2f's 3×3 morphological closing only seals 1-2 pixel gaps; storyboard art commonly has 5-15 pixel gaps where the artist lifted the pen between strokes (head outline doesn't touch torso, arms don't connect to body). The character then fragments into 4-6 connected components and Node 5's reconcile drops the wrong fragments — live-pod test 2026-05-03 on TMKOC EP35 SH004 caught Node 5 detecting bbox=178 px tall when the actual character was ~430 px tall (60% of frame). Phase 2f-tuning shipped `_detect_bboxes_adaptive`: iterative dilation BEFORE CC, trying levels `[0, 1, 2, 3, 5, 7, 10, max_dilation]` and stopping at the first level where significant-blob count ≤ metadata's `expected_count`. Adapts naturally to camera framing — wide shots (small characters with small gaps) need 0-2 dilations; close-ups (large characters with bigger gaps) need 8-15. CLI flag `--bbox-max-dilation N` (default 15) caps the iteration count. New `bbox-adaptive-dilation` warning records the chosen level per-keypose. Schema additions: `CharacterMap` + `Node5Result` gain `bboxMaxDilation: int = 15` (additive, schemaVersion stays at 1). 7 new Node-5 tests (77 total Node-5).
- **7v2-G. Phase 2g — Simplify Node 6: always pick "front" angle when IP-Adapter handles identity (DONE 2026-05-03).** The original 2026-04-23 multi-signal silhouette scoring (silhouette IoU + symmetry + aspect + upper-region edge density across all 8 angles) is RETAINED as opt-in via `--angle-mode auto`, but the default flips to `"front-only"`. Reason: live-pod test 2026-05-03 on TMKOC EP35 SH004 caught Node 6's "auto" mode picking `back-3q-R` (TAPPU from behind) for a front-facing rough — silhouette IoU happened to score highest on the fragmented Node 5 silhouette. IP-Adapter then had only the back of TAPPU's head as the identity reference and produced generic bald-old-man instead of TAPPU. Front-view always shows the character's face, which is what IP-Adapter actually needs. `phase2g_override` marker in `scoreBreakdown` records when the override fires (vs `auto` mode where multi-signal scoring picked "front" legitimately). Schema additions: `Node6Result` gains `angleMode: str = "front-only"` (additive). 9 new Node-6 tests + 1 modified angle test (43 total Node-6).
- **7v2-FixUp3. Phase 2-revision-fixup-3 — runpod_setup.sh x-flux-comfyui auto-patch + Node 11 timeout passthrough + PyTorch cu13 docs (DONE 2026-05-03).** Live-pod-debug bundle for issues that bit us between Phase 2-revision and the user's 3rd live-pod smoke: (a) `runpod_setup.sh` now auto-`sed -i`s `**kwargs` into x-flux-comfyui's `DoubleStreamBlock.forward` + `SingleStreamBlock.forward` signatures right after the `git clone` step. Recent ComfyUI core (>= 0.19) calls these blocks with `attn_mask=...` and `transformer_options=...` keyword args; upstream signatures don't declare them, causing `TypeError: DoubleStreamBlock.forward() got an unexpected keyword argument 'attn_mask'` on every prompt. The patch is idempotent (greps for the pre-patch signature first). (b) Node 11 CLI gained `--per-prompt-timeout SECONDS` flag that passes through to Node 7. The orchestrator's default 600s (10 min) per Flux generation is too short when running on CPU-fallback Flux (~12 min/generation) or just complex shots — we lost ~$2 of pod time when orchestrate.py timed out and marked every detection as `error` even though ComfyUI was still happily generating in the background and the images existed in `output/animatic/`. (c) `runpod_setup.sh` warns at startup if PyTorch < 2.5 detected (the version mismatch with cu13 driver = Flux loads to GPU memory but actual sampling falls back to CPU at 0% GPU util). `tools/POD_NOTES_runpod_slim.md` gained a "Phase 2-revision-fixup-3" section documenting all four issues (this one + the missing-files git-checkout issue + the x-flux-comfyui patch + the timeout passthrough). 3 new Node-11 tests for timeout passthrough (49 total Node-11).
...
**Phase 2-revision-fixup-1 + -fixup-2 shipped 2026-04-28 (post-live-pod-debug bug-fixes for Phase 2-revision).** Live-pod smoke test on a fresh A100 80GB (TMKOC EP35 SH004 fixture; runpod-slim pod) caught two real bugs that all the dry-run + parameterizer-only unit tests missed because they only manifest on the live `_run_one_task` non-dry-run path that contacts ComfyUI's HTTP API:

- **fixup-1:** orchestrate.py's Phase 2-revision bbox-crop block calls `_safe_segment(task.identity)` to construct the per-detection crop filename. `_safe_segment` lives in manifest.py and was already imported by tests/test_node7.py, but orchestrate.py never imported it for its own use. The dry-run path in `_run_one_task` early-returns before hitting the bbox-crop block, so all parameterizer-only tests passed without exercising the import. Live submission triggered `NameError: _safe_segment is not defined` and every Node 7 detection died before any ComfyUI POST. Fix: added `_safe_segment` to orchestrate.py's `from .manifest import (...)` block. New regression test `test_orchestrate_imports_safe_segment_from_manifest` checks the symbol is reachable through orchestrate's namespace (catches the bug at import time without needing a live pod).

- **fixup-2:** workflow_flux_v2.json node 22 had `"provider": "CUDA"` but x-flux-comfyui's `LoadFluxIPAdapter` registers `provider` in INPUT_TYPES as `["CPU", "GPU"]`. ComfyUI's prompt validator rejected every workflow submission with "Value not in list: provider: 'CUDA' not in ['CPU', 'GPU']"; the IP-Adapter never loaded, KSampler never ran, every detection landed as status=error in refined_map.json. Node 8's substitute-rough fallback covered the test (the smoke produced an MP4 from rough copies) but no actual Phase 2 v2 generation occurred. The Phase 2b agent-research mistakenly used `"CUDA"` (the underlying CUDA provider name) instead of `"GPU"` (the value x-flux-comfyui's INPUT_TYPES dropdown actually offers). Fix: changed provider to `"GPU"` in the shipped workflow_flux_v2.json + the test's `_minimal_v2_template` placeholder. New regression test `test_shipped_workflow_flux_v2_ipadapter_provider_is_valid` asserts the shipped JSON's provider is in {'CPU', 'GPU'} and explicitly NOT 'CUDA' — catches at JSON-load time without needing a live pod.

**116 Node-7 tests pass after both fixups (114 pre-fixup + 1 fixup-1 + 1 fixup-2); 481 repo-wide, zero regressions.** The 3rd smoke retry (with both fixups in place) to visually verify Phase 2-revision's per-character BnW generation actually produces BnW per-character output was BLOCKED 2026-04-28 by RunPod credit exhaustion before the smoke could re-run. Resume on a fresh pod with credit recharge: pull HEAD, scp Actual_Testing/, install ComfyUI runtime deps (sqlalchemy/alembic/opencv-python/matplotlib/scikit-image/onnxruntime-gpu — the runpod-slim pod image's pre-installed venv had Comfy's deps but the system Python that runs subprocess'd run_nodeN.py needed them re-installed; documented further down in this PLAN.md), launch ComfyUI, run `python3 run_node11.py --input-dir /workspace/Actual_Testing --work-dir /workspace/test_phase2f_run`, then visually inspect `<shot>/refined/*.png` to confirm Flux generated black-ink-on-white-BG per-character keyposes. Live-pod smoke through Nodes 2-6 + Phase 2f's dark_lines/ artifact already verified 2026-04-28 (88 frames decoded → 3 keyposes → 6 detections → 6 reference matches → 3 dark_lines/ PNGs in 3 sec; the ComfyUI submission path is what the 3rd smoke needs to verify).
