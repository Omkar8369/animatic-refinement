...
**Status (2026-04-26): design-locked, code not yet shipped.** Hug-test proof-of-concept (Flux Dev fp8 + Flat Cartoon Style v1.2 + Union CN + DWPose preprocessor + txt2img, single generation) lives at `_pod_out/flux_tmkoc_test_workflow.json` with output `_pod_out/flux_test/flux_test_tappu_hug_00001_.png` — recognizable Tappu + Champak Lal in TMKOC style, ~70s on the 4090. Phase 2a will use this workflow JSON as the implementation starting point for `workflow_flux_v2.json`. Pickup-after-this-commit: read `CLAUDE.md` "Node 7 v2 — locked decisions" first, then `_pod_out/flux_tmkoc_test_workflow.json`, then start the Phase 2a ship-checklist.
