...
**Phase 2d-run is the next ship-checklist step** — actually train the TMKOC v1 LoRA on a live A100 pod by following `tools/phase2d/PHASE2D_TRAINING_PLAYBOOK.md` (Path A bootstrap → curate → caption → train → validate → ship), then a follow-up commit fills in `models.json`'s `tmkoc-style-v1` URL + sha256 from the trained safetensors weight.
