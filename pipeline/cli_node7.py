...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "poseExtractor). v2 = Phase 2 Flux Dev + ControlNet Union "
            "Pro + XLabs Flux IP-Adapter v2 (single workflow JSON; "
            "txt2img in Phase 2a, img2img with denoise=0.55 from "
            "Phase 2c, per-character bbox crop img2img + BnW line-art "
            "prompts from Phase 2-revision). Default flipped to v2 "
            "in Phase 2c; Phase 1 stays callable via --workflow=v1 "
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "LoRA (Phase 2d-run). Until Phase 2d-run's training "
            "lands a real tmkoc_style_v1.safetensors weight, "
            "--style-lora=tmkoc_v1 will fail at ComfyUI submission "
            "with a missing-file error from node 20's LoraLoader. "
            "See tools/phase2d/PHASE2D_TRAINING_PLAYBOOK.md for the "
            "training procedure. IGNORED when --workflow=v1 (Phase 1 "
            "has its own LoRA stack). Strength is per-LoRA via "
            "STYLE_LORA_STRENGTHS (Phase 2-revision): flat_cartoon_v12 "
            "= 0.0 (LoRA loads but is bypassed because it biases "
            "toward color, conflicting with Part 1's BnW deliverable); "
            "tmkoc_v1 = 0.75 (locked decision #2 production value)."
        ),
    )
    parser.add_argument(
        "--quiet",
