...
| 5    | Character Detection & Position         | **DONE — initial build 2026-04-23; Phase 2f shipped 2026-04-28; Phase 2f-tuning shipped 2026-05-03. 77 tests pass (500 repo-wide); CLI + `run_node5.py` wrapper + ComfyUI wrapper verified; end-to-end Node 2→3→4→5 smoke test passes (Bhim bound to L, Jaggu bound to R on real MP4); classical CC + Strategy A positional identity. Phase 2f replaces the original Otsu binarization with a fixed luminance threshold (default 80, tunable via `--dark-threshold N`) that separates dark character outlines from lighter BG furniture lines per the user's storyboard convention; followed by 3×3 morphological closing to seal small gaps where character outlines crossed BG lines. Side effect: writes `<shot>/dark_lines/<filename>.png` (BnW: black ink on white BG) per keypose; Node 7's `_prepare_rough_bbox_crop` reads from `dark_lines/` when present, falls back to raw keypose otherwise (backward-compat with pre-Phase-2f work dirs). Phase 2f-tuning (post-live-pod-debug 2026-05-03) added `_detect_bboxes_adaptive`: iterative dilation BEFORE CC bridges larger character outline gaps (5-15 pixels typical for storyboard pen lifts) so the character doesn't fragment into multiple CCs and Node 5's reconcile drops the wrong fragments. Adapts to camera framing — wide shots need few/no dilations; close-ups need many; the loop stops as soon as detected blob count ≤ metadata's `expected_count`. CLI flag `--bbox-max-dilation N` (default 15) caps the iteration count; `bbox-adaptive-dilation` warning records the level chosen per keypose.** |
| 6    | Character Reference Sheet Matching     | **DONE — initial build 2026-04-23; Phase 2g shipped 2026-05-03 (front-only angle picker default for IP-Adapter identity). 43 tests pass (500 repo-wide); CLI + `run_node6.py` wrapper + ComfyUI wrapper verified; end-to-end Node 2→3→4→5→6 smoke test passes; alpha-island sheet slicing + Otsu silhouette recompute + 128×128 multi-signal scoring (IoU + symmetry + aspect + upper-region interior-edge density) + DoG/canny/threshold line-art; per-(identity, angle) crop cache; rerun wipes reference_crops/. Phase 2g (locked decision, post-live-pod-debug 2026-05-03): default angle picker mode flipped from `"auto"` (multi-signal scoring across all 8 angles) to `"front-only"` (always pick `"front"`) because IP-Adapter needs the character's FACE for identity learning; back-view crops give it nothing useful and produced generic bald-old-man output instead of TAPPU on the live test. CLI flag `--angle-mode {auto, front-only}` retains the original behavior as opt-in for projects that re-introduce angle-aware identity conditioning later. `phase2g_override` marker in `scoreBreakdown` records when the override fired.** |
| 7    | AI-Powered Pose Refinement             | **Phase 1 DONE (live-verified 2026-04-25). Phase 2a SHIPPED 2026-04-26. Phase 2b SHIPPED 2026-04-27. Phase 2c SHIPPED 2026-04-27 (img2img + v2 is now default). Phase 2d-prep SHIPPED 2026-04-27. Phase 2d-fixup SHIPPED 2026-04-27 (post-live-pod-debug bug-fix: corrected workflow_flux_v2.json class_type strings + models.json IP-Adapter destination path). Phase 2-revision SHIPPED 2026-04-28 (post-design-review: corrected Phase 2c full-frame img2img regression that broke Phase 1 locked decision #5; flipped node 50 to per-character bbox crop; flipped prompts colored TMKOC → BnW line art; bypassed Flat Cartoon LoRA via per-LoRA strength=0.0). Phase 2f SHIPPED 2026-04-28 (Node 5 luminance pre-threshold + morphological closing + dark_lines/ side-effect; Node 7's bbox crop reads dark_lines/ when present so Flux gets character-only pixels with BG furniture erased to white). Phase 2-revision-fixup-1 + -fixup-2 SHIPPED 2026-04-28 (post-live-pod-debug: missing _safe_segment import + LoadFluxIPAdapter provider 'CUDA' → 'GPU'). Phase 2g SHIPPED 2026-05-03 (front-only angle picker default for Node 6; IP-Adapter needs face features for identity). Phase 2f-tuning SHIPPED 2026-05-03 (Node 5 adaptive iterative dilation toward expected_count for character outline gap bridging across all camera framings). Phase 2-revision-fixup-3 SHIPPED 2026-05-03 (post-live-pod-debug: runpod_setup.sh auto-patches x-flux-comfyui DoubleStreamBlock + SingleStreamBlock signatures for **kwargs compatibility with recent ComfyUI core; Node 11 gained --per-prompt-timeout passthrough flag; PyTorch 2.5+/cu13 requirement documented in tools/POD_NOTES_runpod_slim.md to prevent CPU-fallback Flux at 12+ min/generation). Phase 2d-v1-color + Phase 2e-v2 TRAINED 2026-05-12 on B200 (validation PENDING next pod session — credits exhausted before validation step). Three LoRAs in `_pod_out/loras_v2/` locally + `/workspace/ai_toolkit_output_v2/` on EU-RO volume: (1) `tmkoc_style_lora_v1.safetensors` (rank 16, 2500 steps, 370 mixed-character TMKOC EP35 color frames, generic "TMKOC cartoon style, 2D animation" caption — captures the general flat-shaded 2D cartoon aesthetic but is COLOR-trained, NOT the BnW-line-art-specific style LoRA originally envisioned in the Phase 2d-prep playbook; that BnW-trained variant remains a separate future training run on storyboard scene cuts and the placeholder `tmkoc-style-v1` entry in models.json is retained for it); (2) `tappu_lora_v2.safetensors` (rank 32, 3500 steps, 83 manually-cropped per-character TAPPU frames from tools/phase2e/manual_crop.html — operator drew bboxes by hand after YOLO auto-cropper failed at ~16% detection on TMKOC stylized cartoons); (3) `jethalal_lora_v2.safetensors` (rank 32, 3500 steps, 265 manually-cropped per-character JETHALAL frames; saved as v2 on disk because the first JETHALAL training attempt was cancelled mid-flight after diagnosing the cross-character contamination from whole-frame extraction — this v2 is the first complete JETHALAL LoRA shipped, hence `jethalal-lora-v1` in the public models.json entry name). Training data prep pipeline = thumbnail browser (build_thumbnails.py) → selections.json → frame extractor (extract_frames.py) → manual_crop.html drag-bbox tool → manual_bboxes.json → `--apply --force-character X` cropper → `cropped_manual/<char>/*.jpg` → ai-toolkit. The auto_crop_characters.py (YOLO-based) is RETAINED for reference but produces ~16% usable crops on cartoon characters and is bypassed in production by manual_crop.py. Sample images at training time show clear identity capture (TAPPU has the iconic black hair tuft + layered cardigan; JETHALAL has the yellow polo with orange-striped pocket + frustrated hand-on-head pose + TMKOC face proportions); whether stacked-LoRA inference (style + character + ControlNet + IP-Adapter on the real manual_kp1 fixture) produces production-quality BnW output is the OPEN VALIDATION QUESTION pending the next pod session. models.json gained 3 new entries (`tmkoc-style-lora-v1`, `tappu-lora-v2`, `jethalal-lora-v1`) with sha256s pinned + `url: "TODO"` placeholders (weights need upload to HF Hub or operator host before runpod_setup.sh can fetch them on a fresh pod; for now they live on the EU-RO network volume only and validate-on-same-volume works fine).** Phase 1: 47 tests pass; live-verified on RunPod 4090 — both routes generate cleanly via SD 1.5 + AnyLoRA + IP-Adapter Plus. Bringup on the runpod-slim pod image captured in `tools/POD_NOTES_runpod_slim.md`. **Phase 2a (2026-04-26):** Flux + Style LoRA + Union CN integration; 16 locked Phase 2 node IDs in `workflow_flux_v2.json`; `models.json` schema bumped with additive deprecation fields (Phase 1 weights flipped to `deprecated:true` with 2026-10-26 removal); Phase 2 weight pins added (Flux Dev fp16/fp8 + T5-XXL fp16/fp8 + CLIP-L + Flux VAE + Flat Cartoon Style v1.2 + ControlNet Union Pro); `runpod_setup.sh` honours `DOWNLOAD_DEPRECATED` env var; `--workflow {v1,v2}` + `--precision {fp16,fp8}` flags on Node 7 + Node 11 CLIs (passthrough); `RefinedGeneration` + `CharacterSpec` schemas gained additive Phase 2 fields. **Phase 2b (2026-04-27):** Wired XLabs Flux IP-Adapter v2 via 3 new nodes (22 'Load Flux IPAdatpter' [sic — upstream typo preserved verbatim], 23 LoadImage for reference COLOR crop, 24 'Apply Flux IPAdapter'); KSampler's model input rewires from node 20 (style LoRA) to node 24 (IP-Adapter wrapped); `ip_scale=0.8` per locked decision #6; reference image is the COLOR crop from Node 6E (NOT the DoG line-art) per locked decision #4. `models.json` gained 2 new weight pins (`flux-ip-adapter-v2.safetensors` ~1 GB + `clip-vit-large-patch14.safetensors` ~600 MB CLIP-L vision encoder) + 1 new custom-node clone (`x-flux-comfyui` from XLabs-AI). **Phase 2c (2026-04-27): 96 Node-7 tests pass (47 Phase 1 + 49 Phase 2; 441 repo-wide, zero regressions).** TWO architecturally significant flips: (1) `workflow_flux_v2.json` node 80 swapped from `EmptySD3LatentImage` (txt2img) to `VAEEncode` (img2img — wired to take pixels from node 50 rough crop + Flux VAE from node 12); KSampler `denoise` dropped from 1.0 to 0.55 per locked decision #5 — preserves rough composition while regenerating line quality + character identity. (2) `DEFAULT_WORKFLOW` flipped from `"v1"` to `"v2"` in `orchestrate.py` — Phase 2 v2 is now the production default; Phase 1 stays callable via `--workflow=v1` for the entire 6-month deprecation window per locked decision #12. `V2_DENOISE = 0.55` constant added; parameterizer re-asserts it on every generation so a hand-edited JSON can't silently revert to txt2img's denoise=1.0. CLI success line for v2 now reports `precision=<value>` alongside `workflow=v2` (was hidden under v1 because precision is ignored there). 5 new tests cover img2img wiring (node 80 = VAEEncode pixels from 50 + vae from 12), denoise lock (KSampler denoise = 0.55 in shipped JSON + parameterizer re-asserts it), v2-as-default (DEFAULT_WORKFLOW + cfg.workflow + CLI success line all expect v2), and v1 still-callable via flag. 3 existing tests updated to pass `workflow="v1"` explicitly where they meant v1 specifically. **Phase 2d-prep (2026-04-27): 104 Node-7 tests pass (47 Phase 1 + 57 Phase 2; 449 repo-wide, zero regressions).** Shipped the integration infrastructure for Phase 2d's TMKOC v1 style LoRA without yet training the actual safetensors weight. New `--style-lora {flat_cartoon_v12,tmkoc_v1}` flag (default `flat_cartoon_v12`) on `pipeline/cli_node7.py` + `pipeline/cli_node11.py` + ComfyUI wrapper `style_lora` dropdown — parameterizes node 20's `lora_name` based on the choice. New `STYLE_LORA_FILENAMES` table + `STYLE_LORA_CHOICES` + `DEFAULT_STYLE_LORA` constants in `orchestrate.py`; `OrchestrateConfig.__post_init__` validates the field. `tmkoc-style-v1` placeholder entry added to `models.json` with `url: "TODO: replace after the first known-good Phase 2d training run..."` so `runpod_setup.sh` skips it gracefully until a real weight ships; the placeholder documents the contract (destination = `models/loras/tmkoc_style_v1.safetensors`, sizeMB ≈ 80). Two new files in `tools/phase2d/`: `PHASE2D_TRAINING_PLAYBOOK.md` (full Path A bootstrap → curate → caption → train → validate → ship runbook; 6-step procedure with budget estimates) and `ai_toolkit_config_template.yaml` (locked Flux LoRA training params: rank=16, LR=1e-4, 2000 steps, batch_size=1, quantize=true, sample_every=250). 8 new tests cover the flag plumbing (CLI accepts/rejects + default `flat_cartoon_v12` + tmkoc_v1 swap), parameterizer wiring (node 20 `lora_name` flips per choice), config validation (invalid choice raises `ValueError`), and the contract between `STYLE_LORA_FILENAMES` and `models.json` destinations. The Phase 2d-run follow-up (training the actual LoRA on a live A100 pod ~$10-15 + ~6-18 hours of curation/captioning) lives in the playbook and ships separately when it's done. **Phase 2d-fixup (2026-04-27, post-live-pod debug):** Caught two real bugs in Phase 2b that would have blocked Phase 2d-run / Phase 2e-run when actually invoking ComfyUI on a real pod. (1) `workflow_flux_v2.json` had `class_type: "Load Flux IPAdatpter"` and `class_type: "Apply Flux IPAdapter"` — these are the GUI display-name strings from upstream's `NODE_DISPLAY_NAME_MAPPINGS`, not the wire-protocol `NODE_CLASS_MAPPINGS` keys. ComfyUI's workflow-JSON `class_type` field reads the internal class names which are `LoadFluxIPAdapter` and `ApplyFluxIPAdapter` (no spaces, no typo). The original Phase 2b agent-research confused the two; live-pod debug caught it as "node class not found" errors. (2) `models.json` had IP-Adapter destination `models/ipadapter-flux/flux-ip-adapter-v2.safetensors` — but x-flux-comfyui's `folder_paths.folder_names_and_paths['xlabs_ipadapters']` registration scans `models/xlabs/ipadapters/` (`os.path.join(models_dir, 'xlabs', 'ipadapters')`). LoadFluxIPAdapter's `ipadatper` field returned `[]` when querying `/object_info` until the file was moved. Fix updates: workflow_flux_v2.json class_type strings, models.json destination + role description, orchestrate.py constant comments + `_require_node` human-name strings, tests/test_node7.py pinned strings + missing-node regex match, custom_nodes/node_07_pose_refiner/README.md ID table + footer-note explaining wire-protocol class_type vs GUI display name distinction. The `ipadatper` input field name (typo'd) IS real and stays — that's how XLabs registered the field in INPUT_TYPES. 449 tests still pass repo-wide, zero regressions. **Phase 2-revision (2026-04-28, post-design-review):** Corrected the Phase 2c full-frame img2img regression that broke Phase 1 locked decision #5 (per-character generation, NOT whole-frame inpaint) and produced colored TMKOC scenes instead of BnW per-character keyposes. Three architectural flips: (1) `_run_one_task` now pre-crops the keypose to (Node-5 bbox + 20% margin) and resizes to a Flux-compatible canvas (multiples of 16, longest edge ≤ 768) via the new `_prepare_rough_bbox_crop` helper; the crop becomes node 50's input so the pose preprocessor + VAEEncode + KSampler all operate on character-only pixels. The parameterizer now accepts a `rough_image_override` parameter so production runs pass the bbox-crop path while parameterizer-only unit tests fall back to `task.keyPosePath`. (2) `V2_POSITIVE_PROMPT_TEMPLATE` + `V2_NEGATIVE_PROMPT` swapped from "flat cartoon style ... bright daytime colors" + reject "monochrome" → "clean black ink line art, white background, no fill, no color" + reject "color, fill, shading, scene, furniture". The negative prompt no longer rejects "monochrome" (which was Phase 2c's bug — that was rejecting exactly what Part 1 wants). (3) New `STYLE_LORA_STRENGTHS` per-LoRA strength table: `flat_cartoon_v12 → 0.0` (LoRA loads but is bypassed because it biases toward color, conflicting with Part 1's BnW deliverable), `tmkoc_v1 → 0.75` (locked decision #2 production value, applied automatically once Phase 2d-run ships the custom-trained LINE-ART LoRA). The locked decision #2 invariant ("style LoRA at 0.75") survives intact for the LoRA we actually want to use; the placeholder is bypassed via per-LoRA strength rather than removing the LoraLoader chain (which would require rewiring nodes 30 + 24). `workflow_flux_v2.json` node 20 strength updated to `0.0` in JSON; `V2_STYLE_LORA_STRENGTH` constant retained for backwards compat (= STYLE_LORA_STRENGTHS["tmkoc_v1"]). Phase 2d training-data approach also flipped from synthetic Path A bootstrap → user's storyboard scene cuts directly (clean BnW digital lines on white BG, the same target aesthetic v2 produces) — `tools/phase2d/PHASE2D_TRAINING_PLAYBOOK.md` revised end-to-end. **110 Node-7 tests pass** (47 Phase 1 + 63 Phase 2; 6 new Phase 2-revision tests + 1 modified prompt-template test); **455 repo-wide, zero regressions**. **Phase 2f (2026-04-28, Node 5 BG-line fix that Node 7 also benefits from):** Node 5 now extracts character outlines from lighter BG via a fixed luminance threshold (default 80, `--dark-threshold N` to tune) followed by 3×3 morphological closing (seals 1-2 pixel gaps where character outline crossed a BG line). Side effect: each shot grows a `<shot>/dark_lines/<filename>.png` per keypose (white BG, black character ink, BG furniture lines erased). Node 7's `_prepare_rough_bbox_crop` reads from `dark_lines/` when present (falls back to raw keypose for pre-Phase-2f work dirs), so Flux's VAEEncode + ControlNet operate on character-only pixels with no BG furniture for the negative prompt to fight at generation time. 4 new Node-7 tests cover the resolver (prefers dark_lines/, falls back when missing) + end-to-end (bbox crop pulls dark_lines content vs raw keypose). 20 new Node-5 tests cover the luminance helper, closing helper, save/wipe helpers, dark_lines/ output dir + filename matching + polarity + rerun-wipe + character_map.json schema additions + node5_result.json schema additions + CLI flag. **70 Node-5 tests pass + 114 Node-7 tests pass; 479 repo-wide, zero regressions.** **Phase 2g + Phase 2f-tuning + Phase 2-revision-fixup-3 SHIPPED 2026-05-03 (post-live-pod-debug bundle).** Live-pod smoke on TMKOC EP35 SH004 (EU-RO-1 community A100 80GB) caught FOUR more bugs after Phase 2-revision worked architecturally: (i) Node 5's character bbox was 178 pixels tall (25% of frame) instead of the actual ~430 pixels (60% of frame) because storyboard outlines fragmented into 47 CCs (5-15 px gaps between strokes), and reconcile dropped the wrong fragments → Phase 2f-tuning shipped `_detect_bboxes_adaptive` (iterative dilation toward `expected_count`); (ii) Node 6 picked `back-3q-R` for a front-facing rough → IP-Adapter saw only the back of TAPPU's head → output was generic bald-old-man → Phase 2g flipped Node 6's default to `angle_mode="front-only"` so IP-Adapter always sees the character's face; (iii) recent ComfyUI core (>= 0.19) calls Flux blocks with `attn_mask=...` keyword arg but x-flux-comfyui's `DoubleStreamBlock.forward` + `SingleStreamBlock.forward` declared positional-only signatures → `TypeError` on every prompt → fixup-3's `runpod_setup.sh` now sed-patches both signatures to accept `**kwargs` automatically on every fresh pod; (iv) PyTorch 2.4.1+cu124 + driver CUDA 13.0 = Flux loads to GPU memory but actual sampling falls back to CPU at 12+ min/generation → fixup-3 added a `runpod_setup.sh` warning + Node 11's `--per-prompt-timeout` passthrough flag (so when Flux IS slow on CPU the orchestrator doesn't false-time-out at the default 600s and lose the ComfyUI-saved outputs). **77 Node-5 tests + 43 Node-6 tests + 116 Node-7 tests + 49 Node-11 tests pass; 500 repo-wide, zero regressions.** 19 new tests across Node 5/6/11 cover adaptive dilation + front-only override + scoreBreakdown phase2g_override marker + CLI flags + per-prompt-timeout passthrough. **Phase 2-revision-fixup-1 + -fixup-2 (2026-04-28, post-live-pod-debug bug-fixes):** Live-pod smoke test on a fresh A100 80GB caught two real bugs in the previously-shipped Phase 2-revision + Phase 2b code that all the dry-run + parameterizer-only unit tests had missed (because both bugs only manifest on the live `_run_one_task` non-dry-run path that contacts ComfyUI's HTTP API). **fixup-1:** orchestrate.py's _run_one_task uses `_safe_segment(task.identity)` to build the bbox-crop filename, but `_safe_segment` lives in manifest.py and orchestrate.py never imported it; every detection failed with `NameError: _safe_segment is not defined` before any ComfyUI submission. Fix: added `_safe_segment` to orchestrate.py's `from .manifest import (...)` block. New regression test `test_orchestrate_imports_safe_segment_from_manifest` checks the symbol is reachable through orchestrate's namespace + spot-checks it returns the right values. **fixup-2:** workflow_flux_v2.json node 22 had `"provider": "CUDA"` but x-flux-comfyui's LoadFluxIPAdapter registers `provider` in INPUT_TYPES as `["CPU", "GPU"]`. ComfyUI's prompt validator rejected every workflow submission with "Value not in list: provider: 'CUDA' not in ['CPU', 'GPU']"; the IP-Adapter never loaded, KSampler never ran, every detection landed as status=error. Node 8's substitute-rough fallback covered the test (the smoke produced an MP4 from rough copies) but no actual Phase 2 v2 generation occurred. Fix: changed provider to `"GPU"` (the value telling x-flux-comfyui to load the IP-Adapter onto the GPU). New regression test `test_shipped_workflow_flux_v2_ipadapter_provider_is_valid` asserts the shipped JSON's provider is in {'CPU', 'GPU'} and explicitly NOT 'CUDA'. **116 Node-7 tests pass after both fixups (114 pre-fixup + 2 new regression tests); zero regressions repo-wide.** Live-pod smoke verified end-to-end up through the Node 7 ComfyUI submission step (Nodes 2-6 all ran cleanly: 88 frames decoded → 3 keyposes → 6 detections → 6 reference matches; Phase 2f's `<shot>/dark_lines/` directory generated 3 BnW PNGs as expected). 3rd smoke retry to verify Phase 2-revision's per-character BnW generation actually produces BnW per-character output was BLOCKED at 2026-04-28 by RunPod credit exhaustion before the pod could be re-tested with both fixups in place; resume with the 3rd smoke when credits are restored. **Phase 2e + 2g still pending:** 2e (per-character LoRAs trained one-by-one — `CharacterSpec.characterLoraFilename` schema already shipped in 2a) → 2g (Node 6 simplification — make angle picking optional now that IP-Adapter handles identity). |
...
**Phase 2f (2026-04-28) — luminance pre-threshold + dark_lines/
artifact** (do not re-litigate; supersedes the original Otsu-only
binarization for production runs while keeping it available for
tests / future workflows):

6. **Luminance threshold replaces Otsu binarization in production.**
   The original Otsu pipeline (decision #1) treated all dark-vs-light
   pixels as foreground/background, which broke when the artist drew
   BG furniture lines (counters, walls, dashed safe-area rectangles)
   in lighter ink alongside dark-bold-black character outlines —
   Otsu would split BG and characters into one big foreground class,
   and the giant connected component covering 70%+ of the frame
   would either swallow the actual characters or get falsely tagged
   AS the character. The user's storyboard convention puts character
   outlines at luminance ~0-50 and BG at ~80-180; a fixed threshold
   of 80 (default; `--dark-threshold N` to tune) cleanly separates
   them. Locked decision #1's Otsu function is retained in
   `pipeline.node5._binarize_otsu` for tests + debugging but is no
   longer wired into `_detect_on_key_pose` for production.

7. **Morphological closing (3×3) seals outline overlap gaps.** When
   a character outline crosses a BG line, the artist may have drawn
   BG on top at the intersection point, leaving a 1-2 pixel gap in
   the character outline. Without closing, the connected-components
   step would split the character into multiple pieces. `binary_closing`
   (dilate-then-erode by 1 pixel) seals 1-2 pixel gaps without
   merging genuinely separate characters. Kernel size locked at
   3×3 (`DEFAULT_OUTLINE_CLOSING_KERNEL`).

8. **`<shot>/dark_lines/<filename>.png` side-effect for Node 7.**
   The luminance threshold + closing produces a clean binary mask
   (character ink as True, everything else as False). Node 5 saves
   this mask as a PNG with polarity that matches Part 1's deliverable
   (white BG = 255, character ink = 0) — same shape as the input
   keypose, RGB mode for downstream tools' compatibility. Node 7's
   `_prepare_rough_bbox_crop` resolver prefers `dark_lines/<filename>`
   over the raw keypose when present, so Flux's VAEEncode + ControlNet
   operate on character-only pixels instead of mixed character+BG
   ink. Falls back to raw keypose for pre-Phase-2f work dirs (no
   migration needed). `<shot>/dark_lines/` is wiped of stale `*.png`
   on each Node 5 run (mirrors the Nodes 3/4/5/6/8/9 wipe-before-write
   pattern); non-PNG files (operator debug notes) survive the wipe.

9. **Schema additions are additive.** `CharacterMap` (per-shot
   `character_map.json`) gains `darkThreshold: int` + `darkLinesDir:
   str`. `Node5Result` (aggregate `node5_result.json`) gains
   `darkThreshold: int`. Old manifests written before Phase 2f load
   through the new dataclasses unchanged because dataclass defaults
   fill in (`darkThreshold = DEFAULT_DARK_THRESHOLD = 80`,
   `darkLinesDir = ""` meaning "this run pre-dates Phase 2f").
   `schemaVersion` stays at 1.

10. **`_save_dark_lines_png` polarity inverts to white-BG-black-ink.**
    The Node 5 internal binary mask uses True = ink, but the on-disk
    PNG uses 0 (black) = ink + 255 (white) = BG to match Part 1's
    deliverable. Node 7's bbox crop reads back to numpy via
    `Image.open(...).convert("RGB")`; the polarity matches what Flux
    sees on the rough's original orientation.

**Phase 2f-tuning (2026-05-03) — adaptive iterative dilation before CC**
(do not re-litigate; supersedes the implicit "small gaps only" assumption
in decision #7's morphological closing for production runs):

11. **`_detect_bboxes_adaptive` replaces `_detect_bboxes` in
    production.** The Phase 2f closing kernel of 3×3 only seals
    1-2 pixel gaps in character outlines. Storyboard art commonly
    has 5-15 pixel gaps where the artist lifted the pen between
    strokes — head outline doesn't touch torso outline, arms don't
    connect to body, etc. The character then fragments into 4-6
    connected components and Node 5's reconcile drops the wrong
    fragments (live-pod test 2026-05-03 on TMKOC EP35 SH004 caught
    this: 47 components detected for one character; reconcile kept
    the smallest "head" fragment at 25% of frame height instead of
    the full body at 60% of frame height). The fix iteratively
    dilates the binary mask before CC, trying levels
    `[0, 1, 2, 3, 5, 7, 10, max_dilation]` and stopping at the
    first level where significant-blob count ≤ metadata's
    `expected_count`. Adapts naturally to camera framing — wide
    shots have small characters with small gaps (0-2 dilations
    sufficient); close-ups have large characters with large gaps
    (8-15 dilations).

12. **`bbox-adaptive-dilation` warning records the level chosen.**
    The chosen dilation iteration count is appended to the
    per-keypose `warnings` list so the operator can see in
    `character_map.json` exactly how much dilation was needed for
    each frame. Useful diagnostic when output quality drops —
    very high values (> 10) suggest reference-sheet issues or
    extreme close-up framing.

13. **CLI flag `--bbox-max-dilation N`** caps the iteration count
    (default `DEFAULT_BBOX_MAX_DILATION = 15`). Setting lower
    (e.g., 5) helps shots where two characters stand close together
    to avoid over-merging them; higher (e.g., 20) helps extreme
    close-ups with very large character outlines. The
    `_detect_bboxes` (non-adaptive) function is RETAINED for tests
    + future use, but the production `_detect_on_key_pose` always
    uses `_detect_bboxes_adaptive` now.

14. **Schema additions are additive** (schemaVersion stays at 1).
    `CharacterMap` gains `bboxMaxDilation: int = 15`. `Node5Result`
    gains `bboxMaxDilation: int = 15`. Old manifests load through
    the new dataclasses unchanged — defaults fill in the value.
...
**Phase 2g (2026-05-03) — front-only angle picker default**
(do not re-litigate; supersedes locked decision #4's "pick the angle
with max score" for production runs that use IP-Adapter):

11. **Default `angle_mode = "front-only"`** for the production
    Node 7 v2 path. The original 2026-04-23 multi-signal scoring
    (silhouette IoU + symmetry + aspect + upper-region edge density)
    is RETAINED as opt-in via `--angle-mode auto`, but the default
    is now to ALWAYS pick the `"front"` angle. Reason: live-pod
    test 2026-05-03 on TMKOC EP35 SH004 caught Node 6's "auto"
    mode picking `back-3q-R` (TAPPU from behind) for a front-facing
    rough — silhouette IoU happened to score highest on the
    fragmented Node 5 silhouette. IP-Adapter then had only the back
    of TAPPU's head as the identity reference and produced a generic
    bald-old-man instead of TAPPU. Front-view always shows the
    character's face, which is what IP-Adapter actually needs for
    identity learning. The original Phase 2 roadmap entry 7v2-G
    sketched this exact change.

12. **`FORCED_FRONT_ANGLE = "front"`** — must match a name in
    `CANONICAL_ANGLES`. The canonical 8-angle order (locked
    2026-04-23) puts "front" at index 4 (the central position
    facing the camera). The Phase 2g override loops through
    `sheet_crops`, finds the entry where `name == "front"`, and
    sets `best_idx` to that index. If a future sheet layout
    omits the literal `"front"` angle name, the override silently
    no-ops (best_idx falls back to the silhouette-scoring winner).

13. **`scoreBreakdown.phase2g_override` marker.** When the
    override fires, the per-detection `scoreBreakdown` dict in
    `reference_map.json` gains a `"phase2g_override": "front"`
    entry. This lets the operator distinguish "Node 6 picked
    front via Phase 2g override" from "Node 6 picked front
    legitimately via auto-mode silhouette scoring".

14. **Schema additions are additive** (schemaVersion stays at 1).
    `Node6Result` gains `angleMode: str = "front-only"`. Old
    `node6_result.json` files load through the new dataclass
    unchanged — default fills in.

15. **CLI flag `--angle-mode {auto, front-only}`.** Operators can
    opt back to the original behavior on a per-batch basis. The
    Node 6 ComfyUI wrapper exposes the same dropdown.
    Default: `front-only`.
...
**Phase 2-revision (2026-04-28) — corrections to the Phase 2c ship**
(do not re-litigate — these supersede Phase 2c's full-frame img2img
+ colored-output prompts; locked decisions #1-#14 above are otherwise
unchanged):

15. **Node 50 receives a per-character bbox crop, NOT the whole-
    frame keypose.** Phase 2c's "img2img on the rough crop" was
    implemented as VAEEncode of the WHOLE-FRAME keypose; that broke
    Phase 1 locked decision #5 (per-character generation). Phase
    2-revision: `_run_one_task` calls `_prepare_rough_bbox_crop` to
    crop the keypose to (Node-5 bbox + 20% margin) clamped to image
    bounds, resized so longest edge ≤ V2_BBOX_TARGET_MAX_EDGE (768)
    with both dims rounded down to multiples of V2_BBOX_FLUX_MULTIPLE
    (16, Flux requirement). The crop becomes node 50's input — pose
    preprocessor + VAEEncode + KSampler all operate on character-only
    pixels. The parameterizer accepts a `rough_image_override`
    parameter so production passes the bbox-crop path; tests
    fall back to `task.keyPosePath` when no override is provided.

16. **Prompts ask for BnW line art on white BG and reject color**
    (Phase 2c had it backwards). The earlier Phase 2c prompts asked
    for "flat cartoon style ... bright daytime colors" and rejected
    "monochrome" — biased v2 toward color, conflicting with Part 1's
    locked spec ("Output color is Black & White line art"). Phase
    2-revision: `V2_POSITIVE_PROMPT_TEMPLATE` now asks for "clean
    black ink line art ... white background, no fill, no color" and
    `V2_NEGATIVE_PROMPT` rejects "color, fill, shading, scene,
    furniture" (with "monochrome" REMOVED — that was the bug).

17. **Per-LoRA strength override via `STYLE_LORA_STRENGTHS`.** The
    generic Flat Cartoon Style v1.2 LoRA biases toward color, which
    conflicts with Part 1's BnW deliverable. Phase 2-revision adds a
    table mapping LoRA name → strength: `flat_cartoon_v12 → 0.0`
    (LoRA loads but is bypassed; LoraLoader can't be skipped without
    rewiring nodes 30 + 24), `tmkoc_v1 → 0.75` (locked decision #2
    production value, applied automatically once Phase 2d-run ships
    the custom-trained LINE-ART LoRA). Locked decision #2 ("style
    LoRA at 0.75") survives intact for the LoRA we actually want to
    use; the placeholder is bypassed via per-LoRA strength rather
    than removing the LoraLoader chain. `V2_STYLE_LORA_STRENGTH`
    constant retained for backwards compat (= 0.75 = tmkoc_v1's
    strength). `workflow_flux_v2.json` node 20 strength updated to
    0.0 in JSON so a hand-launched workflow (without orchestrate.py
    re-asserting) also bypasses correctly.

18. **Phase 2d training data is the user's storyboard scene cuts**
    (clean digital BnW lines on white BG, characters in shot
    positions, light-line BG furniture, no fill / no color), NOT
    the synthetic Path A bootstrap from Phase 2d-prep's playbook.
    Storyboard cuts are already in the target aesthetic so curation
    is essentially free; captioning emphasizes "TMKOC line art"
    trigger and explicitly drops color references. Per-iteration
    cost drops from ~$10-15 + 12-18 hrs (Path A) to ~$3-5 + 4-8 hrs
    (storyboard cuts). The training procedure (rank=16, LR=1e-4,
    ~2000 steps, ai-toolkit on A100) is unchanged.

Net effect of Phase 2-revision: v2's deliverable matches Phase 1's
deliverable shape (per-character BnW line-art PNGs that Node 8
composites onto a white-BG frame at bbox positions), but with Flux's
superior generation quality + character identity preservation via
XLabs IP-Adapter. The 14 original Phase 2 locked decisions stand;
Phase 2-revision corrects the implementation regression that
contradicted decisions #4 (txt2img/img2img mode), #5 (per-character
generation), and the BnW deliverable from Part 1.
...
| **Phase 2-revision** | Per-character bbox crop + BnW line-art prompts + Flat Cartoon LoRA bypass (post-design-review correction of Phase 2c) | Pre-crop keypose to (bbox + 20% margin) so node 50 receives a per-character image (NOT whole frame); flip prompts from colored TMKOC → BnW line art with explicit "no color, no fill" and reject "color, fill, shading, scene, furniture"; new `STYLE_LORA_STRENGTHS` per-LoRA strength table (flat_cartoon_v12 → 0.0, tmkoc_v1 → 0.75) so the generic LoRA is bypassed without removing the chain; revise Phase 2d playbook to use storyboard scene cuts directly. Locked-decision #2 ("style LoRA at 0.75") survives intact for the LoRA we actually want to use. |
| **2d** | Train TMKOC line-art LoRA | Use storyboard scene cuts as training data (Phase 2-revision approach, NOT Path A synthetic bootstrap); train on A100 with ai-toolkit; ship `tmkoc_style_v1.safetensors` to `models.json`. The custom-trained LoRA gets STYLE_LORA_STRENGTHS["tmkoc_v1"] = 0.75 (locked decision #2) automatically once `--style-lora=tmkoc_v1` is selected. |
...
| **2f** | Fix Node 5 background-line detection bug **(DONE 2026-04-28)** | Replaced Otsu binarization with a fixed luminance threshold (default 80, `--dark-threshold N` to tune) that separates dark character outlines from lighter BG furniture lines per the user's storyboard convention. Added 3×3 morphological closing to seal small outline-overlap gaps. New `<shot>/dark_lines/<filename>.png` side-effect (BnW: black ink on white BG) per keypose; Node 7's bbox crop reads from `dark_lines/` (falls back to raw keypose for pre-Phase-2f work dirs). Original heuristic ("Otsu fallback when bbox spans >70%") was superseded by the user's "dark vs light line" insight which is a real semantic signal in the artwork rather than a fragile size heuristic. 70 Node-5 tests pass + 114 Node-7 tests pass; 479 repo-wide, zero regressions. |
| **Phase 2f-tuning** | Adaptive dilation before CC for character outline gaps **(DONE 2026-05-03)** | Phase 2f's 3×3 closing only seals 1-2 pixel gaps. Storyboard outlines often have 5-15 pixel gaps where the artist lifted the pen between strokes; the character then fragments into 4-6 CCs and reconcile drops the wrong fragments. Phase 2f-tuning shipped `_detect_bboxes_adaptive` which iteratively dilates the binary mask before CC, trying levels `[0, 1, 2, 3, 5, 7, 10, max_dilation]` and stopping at the first level where significant-blob count ≤ metadata's `expected_count`. Adapts to camera framing — wide shots small characters need few/no dilations, close-ups large characters need many. CLI flag `--bbox-max-dilation N` (default 15). New `bbox-adaptive-dilation` warning records the chosen level per-keypose. 7 new Node-5 tests; 77 total Node-5; 500 repo-wide. |
| **Phase 2g** | Simplify Node 6 (always pick "front" angle when IP-Adapter handles identity) **(DONE 2026-05-03)** | Default `angle_mode = "front-only"`. The original 2026-04-23 multi-signal silhouette scoring is RETAINED as opt-in via `--angle-mode auto`. Reason: IP-Adapter needs the character's FACE for identity learning; back-view crops give it nothing useful. Live-pod test 2026-05-03 caught Node 6's "auto" picking `back-3q-R` for a front-facing rough → IP-Adapter saw the back of TAPPU's head → output was generic bald-old-man. `phase2g_override` marker in `scoreBreakdown` records when override fires. 9 new Node-6 tests + 1 modified angle test; 43 total Node-6; 500 repo-wide. |
| **Phase 2-revision-fixup-3** | runpod_setup.sh x-flux-comfyui sed-patch + Node 11 timeout passthrough + PyTorch cu13 docs **(DONE 2026-05-03)** | Live-pod-debug bundle: (a) `runpod_setup.sh` auto-patches x-flux-comfyui's `DoubleStreamBlock` + `SingleStreamBlock` forward signatures to accept `**kwargs` (recent ComfyUI core passes `attn_mask=...` which upstream doesn't declare → TypeError on every prompt). (b) Node 11 CLI gained `--per-prompt-timeout SECONDS` passthrough so orchestrator doesn't false-timeout when Flux is running slowly (CPU fallback or just complex shots). (c) PyTorch 2.5+/cu13 requirement documented in `tools/POD_NOTES_runpod_slim.md` with the "issue-3" symptom (loaded to GPU memory but sampling on CPU at 0% GPU utilization, ~12 min/generation instead of <1 min). `runpod_setup.sh` warns at startup if PyTorch < 2.5 detected. 3 new Node-11 tests for timeout passthrough; 49 total Node-11; 500 repo-wide. |
