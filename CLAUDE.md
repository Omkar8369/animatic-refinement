...
| 7    | AI-Powered Pose Refinement             | **Phase 1 DONE (live-verified 2026-04-25). Phase 2a SHIPPED 2026-04-26 (Flux + Style LoRA + Union CN integration; default still v1 for safety). Phase 2b-2g pending.** Phase 1: 47 tests pass; live-verified on RunPod 4090 — both routes (DWPose + lineart-fallback) generate cleanly via SD 1.5 + AnyLoRA + IP-Adapter Plus. Bringup on the runpod-slim pod image captured in `tools/POD_NOTES_runpod_slim.md`. **Phase 2a (2026-04-26): 84 Node-7 tests pass (47 Phase 1 + 37 new Phase 2; 429 repo-wide, zero regressions on ALL prior nodes).** Shipped: `custom_nodes/node_07_pose_refiner/workflow_flux_v2.json` (16 locked Phase 2 node IDs: 10 UNETLoader / 11 DualCLIPLoader / 12 VAELoader / 20 Style LoraLoader / 30+31 prompts / 40 FluxGuidance / 50 LoadImage / 51 pose preprocessor / 60+61+70 Union CN / 80 latent / 90 KSampler / 100 VAEDecode / 110 SaveImage; node 21 Character LoraLoader reserved for Phase 2e; locked v2 sampler `dpmpp_2m_sde` + scheduler `simple` + 40 steps + FluxGuidance 4.0 + CFG 1.0 + 1280×720 + style-LoRA 0.75 + Union CN 0.65; richer TMKOC-flavoured prompt template); `models.json` schema bumped (additive: `deprecated`/`deprecatedSince`/`deprecatedReason`/`scheduledRemovalDate`/`precision` fields; Phase 1 weights flipped to `deprecated:true` with 2026-10-26 removal date; Phase 2 entries added: Flux Dev fp16/fp8 + T5-XXL fp16/fp8 + CLIP-L + Flux VAE + Flat Cartoon Style v1.2 + ControlNet Union Pro); `runpod_setup.sh` honours `DOWNLOAD_DEPRECATED` env var (default false → fresh pods skip deprecated weights, saves ~6.5 GB); CLI gained `--workflow {v1,v2}` (default `v1` for safety; flip to `v2` happens in Phase 2c) and `--precision {fp16,fp8}` (default `fp16` for A100 80GB; fp8 fallback for 4090) on both `pipeline/cli_node7.py` and `pipeline/cli_node11.py` (passthrough to Node 7 subprocess); `RefinedGeneration` manifest schema gained `workflowName`/`precision`/`characterLoraFilename` (additive — Phase 1 records load with `v1`/`fp8`/`None` defaults so all old `refined_map.json` files still parse); `CharacterSpec` schema in `pipeline/schemas.py` gained `characterLoraFilename`/`characterLoraStrength` (Phase 2e fields, additive — old `characters.json` files load with `None`/0.85 defaults; new validation rejects path-like LoRA filenames); Node 2's `ShotJobCharacter` propagates the new fields into `queue.json` so Node 7 reads one file. Phase 2b-2g still pending: 2b (XLabs IP-Adapter) → 2c (img2img default + flip --workflow default to v2) → 2d (custom-trained TMKOC style LoRA replaces generic Flat Cartoon Style v1.2) → 2e (per-character LoRAs trained one-by-one) → 2f (Node 5 background-line fix) → 2g (Node 6 simplification). |
