...
| 7    | AI-Powered Pose Refinement             | **Phase 1 DONE (live-verified 2026-04-25). Phase 2a SHIPPED 2026-04-26. Phase 2b SHIPPED 2026-04-27. Phase 2c-2g pending.** Phase 1: 47 tests pass; live-verified on RunPod 4090 — both routes (DWPose + lineart-fallback) generate cleanly via SD 1.5 + AnyLoRA + IP-Adapter Plus. Bringup on the runpod-slim pod image captured in `tools/POD_NOTES_runpod_slim.md`. **Phase 2a (2026-04-26):** Flux + Style LoRA + Union CN integration; 16 locked Phase 2 node IDs in `workflow_flux_v2.json`; `models.json` schema bumped (additive deprecation fields); Phase 1 weights flipped to `deprecated:true` (2026-10-26 removal); Phase 2 weights pinned (Flux Dev fp16/fp8 + T5-XXL fp16/fp8 + CLIP-L + Flux VAE + Flat Cartoon Style v1.2 + ControlNet Union Pro); `runpod_setup.sh` honours `DOWNLOAD_DEPRECATED` env var; `--workflow {v1,v2}` (default `v1` for safety; flip in 2c) + `--precision {fp16,fp8}` (default `fp16`) flags on Node 7 + Node 11 CLIs (passthrough); `RefinedGeneration` + `CharacterSpec` schemas gained additive Phase 2 fields. **Phase 2b (2026-04-27): 91 Node-7 tests pass (47 Phase 1 + 44 Phase 2; 436 repo-wide, zero regressions).** Wired XLabs Flux IP-Adapter v2 into `workflow_flux_v2.json` via 3 new nodes (22 'Load Flux IPAdatpter' [sic — upstream typo preserved verbatim], 23 LoadImage for reference COLOR crop, 24 'Apply Flux IPAdapter'); only existing-node change is KSampler's model input rewiring from node 20 (style LoRA) to node 24 (IP-Adapter wrapped model) — additive otherwise per locked decision #10; `ip_scale=0.8` per locked decision #6; reference image is the COLOR crop from Node 6E (NOT the DoG line-art) per locked decision #4. `models.json` gained 2 new weight pins (`flux-ip-adapter-v2.safetensors` ~1 GB at `models/ipadapter-flux/`, `clip-vit-large-patch14.safetensors` ~600 MB at `models/clip_vision/` for the Flux IP-Adapter's vision encoder) + 1 new custom-node clone (`x-flux-comfyui` from XLabs-AI). `orchestrate.py` gained `NODE_FLUX_IPADAPTER_LOADER`/`_REF_IMAGE`/`_APPLY` constants + parameterizer wires `task.referenceColorCropPath` into node 23 + re-asserts `ip_scale=V2_STRENGTH_IP_ADAPTER`. 7 new tests (shipped-JSON IP-Adapter regression guards + class_type typo lock + KSampler model-input wiring + IP-Adapter Apply input wiring + reference-color substitution + ip_scale lock + rough-vs-reference path swap guard + missing-IP-Adapter-node fail-loud). Phase 2c-2g still pending: 2c (img2img default + flip --workflow default to v2) → 2d (custom-trained TMKOC style LoRA replaces generic Flat Cartoon Style v1.2) → 2e (per-character LoRAs trained one-by-one) → 2f (Node 5 background-line fix) → 2g (Node 6 simplification). |
