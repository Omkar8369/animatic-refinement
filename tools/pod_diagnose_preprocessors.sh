...
if [ -z "$PID" ]; then
  echo "  ERROR: nothing listening on 8188. ComfyUI isn't running."
  echo "  Check the pod dashboard or rerun the start script."
  exit 1
fi
...
TMP_JSON=$(mktemp)
HTTP_CODE=$(curl -s -o "$TMP_JSON" -w "%{http_code}" "$COMFY_URL/object_info")

if [ "$HTTP_CODE" != "200" ]; then
  echo "  ERROR: /object_info returned HTTP $HTTP_CODE"
  echo "  body: $(head -c 500 "$TMP_JSON")"
  rm -f "$TMP_JSON"
  exit 1
fi
